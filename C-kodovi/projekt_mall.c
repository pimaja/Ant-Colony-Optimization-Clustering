...
int main(){
    int R=50, N=200, K=5, n=2, maxIterations=2700; //Mislim da u clanku nije definiran R, za R=10 se dobiju najbolji rezultati, isprobavala sam do R=100
    int i, j, k, brojac;
    double percentForLocalSearch = 0.3, percentForPheromonUpdate = 0.03;
    ...
    FILE* stream = fopen("Mall_Customers.csv", "r");
	char line[1024];
	i=0;
    ...
    while (fgets(line, 1024, stream))
	{
		...
		for(j=0; j<n; j++)
        {
            attributes[i][j]=atof(getfield(tmp, j+4));
            tmp = strdup(line);
		}
        i++;
        free(tmp);
    ...
    for(i=0 ; i<N-1 ; i++)
        fprintf(out,"%d," , S[best_ls[0]][i]);
    fprintf(out,"%d" , S[best_ls[0]][N-1]);
    fprintf(out,"\n");
    for(i=0 ; i<N-1 ; i++)
        fprintf(out,"%d," , S[best_ls[0]][i]);
    fprintf(out,"%d" , S[best_ls[0]][N-1]);
